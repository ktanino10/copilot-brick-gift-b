...
from verify_build_log import JournalDocument, verify_user_video_link
from user_video import ONLINE_VIDEO_LINKS
...
class UserVideoLinkTests(unittest.TestCase):
    ...
    def test_exact_safe_user_link_is_allowed(self):
        verify_user_video_link(self.document(self.markup()))

    def markup(self):
        return "".join(f'<a href="{url}" target="_blank" rel="noopener noreferrer">Video</a>'
                       for url in ONLINE_VIDEO_LINKS)

    def test_missing_safety_or_changed_tracking_url_is_rejected(self):
        with self.assertRaisesRegex(ValueError, "safe new-tab"):
            verify_user_video_link(self.document(self.markup().replace(' rel="noopener noreferrer"', "", 1)))
        with self.assertRaisesRegex(ValueError, "exact"):
            verify_user_video_link(self.document(self.markup().replace(
                "https://youtu.be/Lc_enNE3nng", "https://youtu.be/Lc_enNE3nng?tracking=1"
            )))
