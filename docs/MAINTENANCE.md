...
公開後のPages検査は既存Linux CIで行い、9/24追記ではHTTP版に加えて
...
9/23の9枚を置き換えず、9/24の7枚を別folderへ追加します。メタデータ・リンク・16画像の実decodeを確認し、
今回の追記前に保存した147件の印刷／native／図面／3D媒体／旧写真のhashと照合します。
進捗の直行リンクは`BUILD-LOG.html#progress-2026-09-24`です。
